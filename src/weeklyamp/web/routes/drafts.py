...
@router.post("/quick-draft", response_class=HTMLResponse)
async def quick_draft(request: Request, topic: str = Form(...), section_slug: str = Form(""), word_count: int = Form(300)):
    """Paste a topic, AI writes a complete section instantly."""
    repo = get_repo()
    config = get_config()

    prompt = f"Write a newsletter section about: {topic}\n\nTarget length: {word_count} words.\nBe engaging, informative, and cite sources where relevant."
    if section_slug:
        prompt = f"Write a newsletter section for the '{section_slug}' section about: {topic}\n\nTarget: {word_count} words."

    content, model = generate_draft(prompt, config, max_tokens_override=word_count * 2)

    if content:
        return HTMLResponse(f'<div class="card" style="margin-top:12px"><h4>Quick Draft: {topic[:50]}</h4><div style="white-space:pre-wrap;font-size:14px;line-height:1.7;max-height:400px;overflow-y:auto;">{content}</div><p style="margin-top:12px;font-size:12px;color:var(--text-dim);">Generated by {model}</p></div>')
    return HTMLResponse('<div class="alert alert-warning">Draft generation failed. Check AI provider configuration.</div>')
